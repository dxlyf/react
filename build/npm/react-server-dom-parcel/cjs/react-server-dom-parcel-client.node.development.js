/**
 * @license React
 * react-server-dom-parcel-client.node.development.js
 *
...
"production" !== process.env.NODE_ENV &&
  (function () {
    function resolveServerReference(bundlerConfig, ref) {
      ...
      return [id, idx, bundlerConfig];
    }
    function preloadModule(metadata) {
      metadata[3] && parcelRequire.extendImportMap(metadata[3]);
      return 0 === metadata[2].length
        ? null
        : Promise.all(
            metadata[2].map(function (url) {
    ...
    function prepareDestinationWithChunks(moduleLoading, bundles, nonce) {
      for (moduleLoading = 0; moduleLoading < bundles.length; moduleLoading++) {
        ...
        JSCompiler_temp_const.M.call(
          JSCompiler_temp_const,
          parcelRequire.meta.publicUrl + bundles[moduleLoading],
          { crossOrigin: void 0, nonce: nonce }
        );
      }
    }
    ...
    function resolveModule(response, id, model, streamState) {
      var chunks = response._chunks,
        chunk = chunks.get(id),
        clientReferenceMetadata = JSON.parse(model, response._fromJSON);
      prepareDestinationWithChunks(
        response._moduleLoading,
        clientReferenceMetadata[2],
        response._nonce
      );
      if ((model = preloadModule(clientReferenceMetadata))) {
        ...
        chunk
          ? (resolveChunkDebugInfo(streamState, chunk),
            resolveModuleChunk(response, chunk, clientReferenceMetadata))
    ...
    function findSourceMapURL$1(filename, environmentName) {
      var devServer = parcelRequire.meta.devServer;
      if (null != devServer) {
